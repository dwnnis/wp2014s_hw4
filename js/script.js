...
//以下為canvas的程式碼，基本上不需多動，依據comments修改即可
    ...
    function handleMouseMove(e){//滑鼠移動的event
      ...
      if(isDragging){ //當拖拉為True時
			...
			ctx.drawImage(profileIMG,canMouseX+50,canMouseY+50);//從XY軸0，0值開始畫如profileimg
			
			var inputedText = $('#inputed').val();//抓取頁面inputed ID的內容
			ctx.fillStyle = "black"; //字體顏色
			ctx.font='20px "微軟正黑體"'; //字體大小和字形
			ctx.fillText(inputedText, 180,440); //字體也可以依據滑鼠游標移動，所輸入的值可自行調整，若不想移動輸入的字體，可以把它改成（inputedText,0,0)X Y軸 0，0的位置
      }
    }
